...
function admin_breadcrumb($arr_breadcrumb=null)
{
	...
	$str_breadcrumb	=	'<div><ul class="breadcrumb"><li><a href="'.admin_base_url().'">'.addslashes(t("Home")).'</a></li>' ;
	...
	if(!empty($arr_breadcrumb))
	{
		...
		foreach($arr_breadcrumb as $key=>$val)
		{
			if(!is_array($val))
			{
				$str_breadcrumb .= '<li><a href="javascript:void(0);">'.$val.'</a></li>';
			}
			else{
